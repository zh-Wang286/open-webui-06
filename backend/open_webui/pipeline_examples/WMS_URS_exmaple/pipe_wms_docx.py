...
输出格式: JSON对象，包含以下字段:
{{
  "covered_fields": [已覆盖需求点列表],
  "missing_fields": [缺失需求点列表],
  ...
  "next_steps": "下一步建议"
            ...
            try:
                ...
                feedback = (
                    ...
                    f"### 已覆盖需求点 ({len(analysis_result.get('covered_fields', []))}):\n"
...
<head>
    <meta charset="UTF-8">
    <title>WMS系统用户需求说明</title>
</head>
<body>

    <!-- 文档元数据 -->
    <div class="metadata">
        <h1>{{ doc_metadata.title }}</h1>
        <p><strong>文档编号：</strong>{{ doc_metadata.document_id }}</p>
        <p><strong>版本：</strong>{{ doc_metadata.version }}</p>
        <p><strong>日期：</strong>{{ doc_metadata.date }}</p>
    </div>
    ...
    <table>
        ...
        <tr>
            <td>{{ abbr.abbr }}</td>
            <td>{{ abbr.explanation_zh }}</td>
            <td>{{ abbr.explanation_en }}</td>
        </tr>
        {{% endfor %}}
    </table>
    ...
    <h3>{{ req.category }} - {{ req.item_id }}: {{ req.item_name }}</h3>
    <table>
        <tr>
            <td width="20%"><strong>关键性</strong></td>
...
"""
            ...
            docx_content = None
            try:
                log.info("Converting HTML to DOCX using pypandoc...")
                ...
                log.info(f"Writing HTML content to temporary file: {temp_html_path}")
                with open(temp_html_path, 'w', encoding='utf-8') as f:
                    f.write(json_str)

                log.info(f"Converting HTML to DOCX: {temp_html_path} -> {temp_docx_path}")
                # 使用pypandoc进行转换
                output = pypandoc.convert_file(
                    source_file=temp_html_path,
                    format='html',
                    to='docx',
                ...
                log.info("Conversion completed, reading DOCX content")
                # 读取生成的DOCX文件
                if os.path.exists(temp_docx_path):
                    with open(temp_docx_path, 'rb') as docx_file:
                        docx_content = docx_file.read()
                    log.info(f"Successfully read DOCX content, size: {len(docx_content)} bytes")
                else:
                    raise FileNotFoundError(f"Expected DOCX file not found at {temp_docx_path}")

            except Exception as e:
                log.error(f"Error converting HTML to DOCX with pypandoc: {str(e)}")
                log.error(f"Error details:", exc_info=True)
                save_message = f"\\n\\n注意: HTML 到 DOCX 转换失败。错误信息: {str(e)}"
            finally:
                # 清理临时文件和目录
                try:
                    ...
                    log.warning(f"Failed to clean up temporary files: {cleanup_error}")

            # 更新状态
            state["document_content"] = json_str  # Store the original HTML content
            state["stage"] = "complete"  # 完成分析

            # 创建文档文件名（使用时间戳确保唯一性）
            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
            filename = f"wms_urs_document_{timestamp}.docx"
            ...
            # 设置静态资源目录路径
            static_resource_dir = os.path.join(backend_dir, "static", "wms_docs")
            os.makedirs(static_resource_dir, exist_ok=True)
            log.info(f"创建静态资源目录: {static_resource_dir}")

            # 完整的文件路径
            output_file = os.path.join(static_resource_dir, filename)
            
            # 保存文件
            if docx_content:
                with open(output_file, "wb") as f:
                    f.write(docx_content)
                log.info(f"Successfully saved DOCX file to: {output_file}")
            else:
                raise ValueError("No DOCX content available to save")

            # 设置正确的URL路径
            url_path = f"/static/wms_docs/{filename}"
            ...
            save_message = (
                ...
                if docx_content else "\\n\\n错误：DOCX文档生成和保存均失败（转换步骤出错）。"
            )

            # 生成文档摘要作为响应
            response = (
                f"## WMS系统用户需求说明生成完成\\n\\n"
                f"DOCX文档已处理。\\n" # Simplified summary for DOCX
                f"{save_message}"
            )
